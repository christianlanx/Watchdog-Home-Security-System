...
from bme280 import BME280       # import the class object for our BME280 sensor
    ...
    def __init__(self):
        ...
        if self.readfrom == 'unset':
            try:
                ...
                print('BME280 not found on 0x76, trying 0x77')
            else:
                print('BME280 found on 0x76')
                self.readfrom = 'bme680secondary'
                # self.sensor = BME680(self.readfrom)
                self.sensor = BME280(self.readfrom)  # Make the class' sensor BME280
        ...
        if self.readfrom == 'unset':
            try:
                ...
                print('BME280 not found on 0x77')
            else:
                print('BME280 found on 0x77')
                self.readfrom = 'bme680primary'
                # self.sensor = BME680(self.readfrom)   <---- OLD; working
                self.sensor = BME280(self.readfrom)     # Make the class' sensor BME280
...
class balenaSenseHTTP(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # print("##############\tHERE ARE THE MEASUREMENTS MAN\t##############")
        # print(measurements)
        # print("##############\t############################\t##############")
        print("Sending measurements in JSON")
